...
export function Captions() {
	...
	const handleGenerateTranscript = async () => {
		try {
			...
			// Index segments in Axum backend RAG vector database
			try {
				const API_URL = process.env.NEXT_PUBLIC_API_URL || "http://localhost:8000";
				await fetch(`${API_URL}/api/ai/index-chunks`, {
					method: "POST",
					headers: {
						"Content-Type": "application/json",
					},
					body: JSON.stringify({
						projectId: activeProject?.metadata.id || "default",
				...
				console.log("Successfully indexed transcript chunks for RAG");
			} catch (e) {
				console.warn("Failed to index transcript chunks for RAG:", e);
			}
