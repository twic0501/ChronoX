...
function PreviewCanvas({
	...
	const triggerThrottledVisionQA = useCallback((sourceCanvas: HTMLCanvasElement) => {
		const now = Date.now();
		if (now - lastVisionCheckTimeRef.current < 4000) return; // 4 seconds throttle
		lastVisionCheckTimeRef.current = now;

		// Check if any visual edit/chroma effect exists on timeline
		const tracks = editor.timeline.getTracks();
		const hasVisualEffects = tracks.some((t) => 
			t.type === "effect" && t.elements.some((el) => {
				const name = el.name.toLowerCase();
				return name.includes("chroma") || name.includes("inpaint") || name.includes("mask");
			})
		);
		if (!hasVisualEffects) return;

		// Downscale canvas to 256x256
		...
		if (ctx) {
			...
			// Async call to backend Vision endpoint
			const API_URL = process.env.NEXT_PUBLIC_API_URL || "http://localhost:8000";
			fetch(`${API_URL}/api/ai/vision-check`, {
				method: "POST",
				headers: {
			...
			.then((data) => {
				if (data && data.ok === false) {
					toast.warning("Quality warning from Vision AI", {
						description: "Detected distorted/blurred edges near the center of the frame. Consider increasing the mask's Feather.",
						duration: 4000,
					});
				}
			})
			.catch((err) => console.warn("Vision QA check error:", err));
		}
	}, [editor]);
				...
				renderer
					...
					.then(() => {
						...
						if (canvasRef.current) {
							triggerThrottledVisionQA(canvasRef.current);
						}
					});
			}
		}
	}, [renderer, renderTree, editor.playback, triggerThrottledVisionQA]);
