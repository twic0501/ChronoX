...
import { OcMirrorIcon, OcShapesIcon } from "@chronox/ui/icons";
...
function MaskItem({
	...
	const handleAutoTrack = async () => {
		...
		const strokes = JSON.parse(mask.params.points || "[]");
		if (strokes.length === 0) {
			toast.error("Please draw a brush scribble on the preview canvas first.");
			return;
		}

		setIsTracking(true);
		toast.info("Starting AI Tracking...", {
			description: "YOLOv12 & OSNet are processing the video.",
		});

		try {
			const API_URL =
				process.env.NEXT_PUBLIC_API_URL || "http://127.0.0.1:8000";
			const response = await fetch(`${API_URL}/api/ai/track`, {
				method: "POST",
				headers: { "Content-Type": "application/json" },
				body: JSON.stringify({
					media_id: "mediaId" in element ? (element as any).mediaId : "",
					points: strokes,
					brush_size: mask.params.brushSize,
					keyframe_time: currentTime - element.startTime,
					clip_start: element.trimStart,
					clip_duration: element.duration,
				}),
			});

			if (!response.ok) {
				throw new Error("Failed to track object");
			}

			const data = await response.json();
			if (data.status === "success" && data.tracking_path) {
				editor.timeline.updateElements({
					updates: [
						{
							trackId,
								...
								masks: [
									{
										...
										params: {
											...mask.params,
											trackingPath: JSON.stringify(data.tracking_path),
										},
									},
								],
							} as Partial<MaskableElement>,
				...
				toast.success("AI Tracking completed successfully!", {
					description: `Tracked ${data.tracking_path.length} frames.`,
				});
			} else {
				throw new Error(data.message || "Failed to track object");
			}
		} catch (e) {
			console.error("AI Tracking failed:", e);
			toast.error("AI Tracking failed", { description: String(e) });
		} finally {
			setIsTracking(false);
		}
	};
