...
import { OcShapesIcon, OcVideoIcon } from "@chronox/ui/icons";
import {
	ContextMenu,
	ContextMenuContent,
	ContextMenuItem,
...
export function Timeline() {
	...
	const isTimelineLocked = useEditorStore((s) => s.isTimelineLocked);
	const {
		selectedElements,
	...
	return (
		<section
			className={cn(
				...
				isTimelineLocked && "opacity-70 pointer-events-none select-none"
			)}
			{...dragProps}
			aria-label="Timeline"
		>
			{isTimelineLocked && (
				<div className="absolute inset-0 bg-background/20 backdrop-blur-[1px] z-50 flex items-center justify-center pointer-events-auto">
					<div className="bg-popover/90 border border-agent/25 rounded-lg px-4 py-2.5 flex items-center gap-2 shadow-xl text-xs font-semibold text-agent select-none animate-pulse">
						<div className="size-2 rounded-full bg-agent animate-ping" />
						<span>AI is editing timeline...</span>
					</div>
				</div>
			)}
			<TimelineToolbar
