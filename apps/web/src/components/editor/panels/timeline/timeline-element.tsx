...
class SequentialThumbnailQueue {
...
const thumbnailQueue = new SequentialThumbnailQueue();

class TimelineThumbnailManager {
	...
	async getFrame({
		...
		let entry = this.sinks.get(mediaId);
		if (!entry) {
			try {
				...
				console.error("Failed to initialize timeline sink:", e);
				return null;
			}
		}

		try {
			return await entry.sink.getSample(time);
		} catch (e) {
			console.warn(`Failed to get sample at ${time}s:`, e);
			return null;
		}
	}
}

const timelineThumbnailManager = new TimelineThumbnailManager();
...
function TimelineTile({ mediaId, file, time, width, height, fallbackUrl }: TimelineTileProps) {
	...
	useEffect(() => {
		...
		const loadFrame = async () => {
			try {
				const frame = await thumbnailQueue.add(async () => {
					if (!active) return null;
					return await timelineThumbnailManager.getFrame({ mediaId, file, time });
				});
				...
				try {
					...
					tempCanvas.width = width * 1.5; // Render slightly higher resolution for clarity
					tempCanvas.height = height * 1.5;
					const ctx = tempCanvas.getContext("2d");
					if (ctx) {
						frame.draw(ctx, 0, 0, tempCanvas.width, tempCanvas.height);
				...
				console.warn("Failed to generate tile thumbnail:", e);
			}
		};
	...
	}, [mediaId, file, time, width, height]);
	...
	return (
		<div
			style={{
				...
				backgroundImage: `url(${imgUrl || fallbackUrl})`,
...
function TiledMediaContent({
	...
	if (element.type === "video" && mediaAsset?.file) {
		...
		const videoDuration = mediaAsset.duration || 99999;
		const rate = (element as VideoElement).retime?.rate ?? 1;
		const pixelsPerSecond = 50 * zoomLevel; // TIMELINE_CONSTANTS.PIXELS_PER_SECOND is 50
		const elementWidth = duration * pixelsPerSecond;
		const numTiles = Math.ceil(elementWidth / tileWidth);
		const tiles = [];

		for (let i = 0; i < numTiles; i++) {
			const tileOffsetTime = (i * tileWidth) / pixelsPerSecond;
			const time = Math.min(element.trimStart + tileOffsetTime * rate, videoDuration);
			tiles.push(
				<TimelineTile
					key={`${element.id}-tile-${i}-${time}`}
