...
if (typeof window !== "undefined" && "VideoDecoder" in window) {
	try {
		...
		// @ts-ignore
		window.VideoDecoder = class PatchedVideoDecoder extends OriginalVideoDecoder {
			configure(config: VideoDecoderConfig) {
				const patchedConfig = {
					...config,
					hardwareAcceleration: "prefer-software" as const,
				};
				super.configure(patchedConfig);
			}
		};
	} catch (e) {
		console.warn("Failed to patch VideoDecoder:", e);
	}
}
...
import { TranscodeModal } from "@/components/editor/modals/transcode-modal";
