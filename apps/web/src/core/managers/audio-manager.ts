...
export class AudioManager {
	private vadCache = new Map<string, Array<{ start: number; end: number }>>();
	...
	public async preloadVadSegments(): Promise<void> {
		const tracks = this.editor.timeline.getTracks();
		for (const track of tracks) {
			if (track.name.toLowerCase().includes("bgm") || track.name.toLowerCase().includes("music")) {
				continue;
			}
			for (const el of track.elements) {
				if (this.vadCache.has(el.id)) continue;
				try {
					const res = await fetch(`http://127.0.0.1:8000/api/elements/${el.id}/vad`);
					if (res.ok) {
						const segs = await res.json();
						this.vadCache.set(el.id, segs);
					}
				} catch (e) {
					console.warn(`Failed to preload VAD for element ${el.id}:`, e);
				}
			}
		}
	}
	...
	private handlePlaybackChange = (): void => {
		...
		if (isPlaying !== this.lastIsPlaying) {
			...
			if (isPlaying) {
				void this.preloadVadSegments().then(() => {
	...
	private buildDuckingPoints(
		clip: AudioClipSource,
		basePoints: Array<{ localTime: number; gain: number }>
	): Array<{ localTime: number; gain: number }> {
		const tracks = this.editor.timeline.getTracks();
		const track = tracks.find((t) => t.elements.some((el) => el.id === clip.id));
		const isBgm = track ? (track.name.toLowerCase().includes("bgm") || track.name.toLowerCase().includes("music")) : false;
		...
		for (const t of tracks) {
			...
			for (const el of t.elements) {
				const segs = this.vadCache.get(el.id);
				if (segs && segs.length > 0) {
					const rate = (el as any).retime?.rate ?? 1.0;
					const trimStart = (el as any).trimStart ?? 0;
					for (const seg of segs) {
						const startInClip = seg.start - trimStart;
						const endInClip = seg.end - trimStart;
						const globalStart = el.startTime + startInClip / rate;
						const globalEnd = el.startTime + endInClip / rate;

						const clampedStart = Math.max(el.startTime, globalStart);
						const clampedEnd = Math.min(el.startTime + el.duration, globalEnd);

						if (clampedEnd > clampedStart) {
							voiceIntervals.push({ start: clampedStart, end: clampedEnd });
						}
					}
				}
			}
		...
		const bgmRate = clip.retime?.rate ?? 1.0;
		return basePoints.map((p) => {
			const pGlobal = clip.startTime + (p.localTime - clip.trimStart) / bgmRate;
			let isOverlapping = false;
			for (const interval of voiceIntervals) {
				if (pGlobal >= interval.start - 0.3 && pGlobal <= interval.end + 0.3) {
					isOverlapping = true;
					break;
				}
			}
			return {
				...
				gain: isOverlapping ? p.gain * 0.25 : p.gain,
			};
		});
	}
	...
	private scheduleClipGainAutomation({
		...
		if (!hasAnimatedVolume({ element: clip.timelineElement }) && !isBgm) {
			return;
		}

		let points = buildAudioGainAutomation({
			element: clip.timelineElement,
			fromLocalTime: startLocalTime,
			toLocalTime: clip.duration,
		});

		points = this.buildDuckingPoints(clip, points);
