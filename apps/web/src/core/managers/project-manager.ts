...
export class ProjectManager {
	...
	async createNewProject({
		...
		const newProject: TProject = {
			metadata: {
				...
				createdAt: new Date(),
				updatedAt: new Date(),
				...(aiBrief?.trim() ? { aiBrief: aiBrief.trim() } : {}),
			},
			scenes: [mainScene],
	...
	private async updateThumbnailFromTimeline(): Promise<boolean> {
		...
		let isBlack = true;
		const checkTimes = [0, 1, 2, 5, duration / 2];

		for (const checkTime of checkTimes) {
			if (!Number.isFinite(checkTime) || checkTime > duration) continue;
			...
			if (ctx) {
				try {
					...
					console.warn("Could not check project thumbnail canvas pixels:", e);
					isBlack = false;
				}
			} else {
				isBlack = false;
		...
		if (isBlack || !thumbnailDataUrl) {
			thumbnailDataUrl = tempCanvas.toDataURL("image/png"); // Fallback
		}
