...
export function useTimelineDragDrop({
	...
	const executeMediaDrop = useCallback(
		({ target, dragData }: { target: DropTarget; dragData: MediaDragData }) => {
			...
			if (!mediaAsset) {
				// @ts-ignore
				if (dragData.mediaType === "audio" && dragData.sourceUrl) {
					const duration =
						// @ts-ignore
						dragData.duration ?? TIMELINE_CONSTANTS.DEFAULT_ELEMENT_DURATION;
			...
			const element = buildElementFromMedia({
				...
				duration,
				startTime: target.xPosition,
				sourceOriginalPath: (mediaAsset.file as any).originalPath,
				sourceProxyPath: (mediaAsset.file as any).proxyPath,
			});
						...
						const element = buildElementFromMedia({
							...
							buffer:
								createdAsset.type === "audio"
									...
									: undefined,
							sourceOriginalPath: (createdAsset.file as any).originalPath,
							sourceProxyPath: (createdAsset.file as any).proxyPath,
						});
