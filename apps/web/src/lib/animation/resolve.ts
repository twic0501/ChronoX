...
export function resolveTransformAtTime({
	...
	// Heal malformed transforms persisted by older code that wrote
	// { scale: {x,y}, rotation } — undefined scaleX would otherwise produce a
	// NaN canvas size and kill the render loop (frozen playback).
	const legacy = baseTransform as any;
	const baseScaleX = Number.isFinite(baseTransform.scaleX)
		? baseTransform.scaleX
		: Number.isFinite(legacy?.scale?.x)
			? legacy.scale.x
			: 1;
	const baseScaleY = Number.isFinite(baseTransform.scaleY)
		? baseTransform.scaleY
		: Number.isFinite(legacy?.scale?.y)
			? legacy.scale.y
			: 1;
	return {
		position: getVectorChannelValueAtTime({
			...
			time: safeLocalTime,
			fallbackValue: baseTransform.position ?? { x: 0, y: 0 },
		}),
		...
		rotate: getNumberChannelValueAtTime({
			...
			time: safeLocalTime,
			fallbackValue: Number.isFinite(baseTransform.rotate)
				? baseTransform.rotate
				: Number.isFinite(legacy?.rotation)
					? legacy.rotation
					: 0,
		}),
	};
