...
function buildUploadAudioElement({
	...
	if (sourceOriginalPath) {
		(element as any).sourceOriginalPath = sourceOriginalPath;
	}
	if (sourceProxyPath) {
		(element as any).sourceProxyPath = sourceProxyPath;
	}
	return element;
}
...
export function buildElementFromMedia({
	...
	switch (mediaType) {
		...
		case "image":
			return buildImageElement({ mediaId, name, duration, startTime }) as any;
	}
}
