...
class EffectPreviewService {
	...
	private getOrCreatePreviewContext({
		...
		if (!this.previewCanvas || !this.previewGl) {
			...
			// Prefer WebGL2 to match the main renderer — some effect shaders
			// (e.g. LUT grade) are #version 300 es and fail on a WebGL1 context.
			...
			if (!this.previewGl) {
				throw new Error("WebGL/WebGL2 not supported");
			}
		}
