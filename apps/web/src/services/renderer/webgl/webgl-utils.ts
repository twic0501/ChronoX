...
export function compileProgram({
	...
	const isWebGL2Shader = fragmentShaderSource.trim().startsWith("#version 300 es");
	const vertexShaderSource = isWebGL2Shader
		? `#version 300 es
in vec2 a_position;
out vec2 v_texCoord;
void main() {
...
export function applyMultiPassEffect({
	...
	for (let i = 0; i < passes.length; i++) {
		...
		if (pass.extraBindings) {
			for (const binding of pass.extraBindings) {
				const gl2 = context as any;
				const targetBind = binding.is3D ? (gl2.TEXTURE_3D || 0x806F) : context.TEXTURE_2D;
				context.activeTexture(context.TEXTURE0 + binding.unit);
				context.bindTexture(targetBind, binding.texture);
				const loc = context.getUniformLocation(program, binding.name);
				if (loc !== null) {
					context.uniform1i(loc, binding.unit);
				}
			}
		}
